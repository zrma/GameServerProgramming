...
{
	//TODO: header에 MemAllocInfo를 펼친 다음에 실제 앱에서 사용할 메모리 주소를 void*로 리턴... 실제 사용되는 예 및 DetachMemAllocInfo 참고.  -> 구현
	// return 0;

	// header->mAllocSize = size;
	new (header)MemAllocInfo( size ); // 생성자를 사용해서 초기화 하도록 하라! by sm9
...
T* xnew(Args... arg)
{
	...
	//TODO: T* obj = xnew<T>(...); 처럼 사용할 수있도록 메모리풀에서 할당하고 생성자 불러주고 리턴.  -> 구현
	void* alloc = nullptr; 
...
void xdelete(T* object)
{
	...
	//TODO: object의 소멸자 불러주고 메모리풀에 반납.  -> 구현	
	object->~T();
